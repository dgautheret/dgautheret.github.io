---
layout: default
title: Home
---
# Daniel Gautheret
**Professor of Bioinformatics** Institute for Integrative Biology of the Cell (I2BC)  
Université Paris-Saclay, CNRS, CEA  
Gif-sur-Yvette, France
---

---

## Research Overview

My research stands at the intersection of **computational transcriptomics, non-coding RNA (ncRNA) biology, and oncology**. The core mission of my laboratory is to develop innovative, reference-free computational methods capable of analyzing massive RNA-seq datasets without the bias of standard genomic annotations. 

By utilizing k-mer-based alignment-free algorithms, our tools capture hidden transcriptional events—such as unannotated long non-coding RNAs (lncRNAs), human endogenous retroviruses (HERVs), and cryptic splicing variations—that are systematically missed by traditional pipelines. We apply these approaches directly to clinical oncology to identify predictive biomarkers for immunotherapy and chemotherapy resistance, and we delve into RNA regulation mechanisms to map complex host-pathogen interactions and intracellular mRNA dynamics.

---
## Selected Recent Publications

* **Reference-free RNA profiling predicts triple negative breast cancer chemoresistance to neoadjuvant treatment.** Torossian N, ..., **Gautheret D**, Morillon A.
  *NAR Cancer* (2025). [DOI: 10.1093/narcan/zcaf036](https://doi.org/10.1093/narcan/zcaf036)
    *Application of reference-free transcript fragments to predict therapy outcomes and bypass annotation bias.*

* **Deciphering human endogenous retrovirus expression in colorectal cancers: exploratory analysis regarding prognostic value in liver metastases.** Viot J, ..., Borg C, **Gautheret D**.
  *EBioMedicine* (2025). [DOI: 10.1016/j.ebiom.2025.105727](https://doi.org/10.1016/j.ebiom.2025.105727)
    *Uncovering the early oncogenic footprint and prognostic value of specific HERV loci in metastatic colorectal cancer.*

* **KaMRaT: a C++ toolkit for k-mer count matrix dimension reduction.** Xue H, Gallopin M, Marchet C, Nguyen HN, Wang Y, Lainé A, Bessiere C, **Gautheret D**.
  *Bioinformatics* (2024). [DOI: 10.1093/bioinformatics/btae090](https://doi.org/10.1093/bioinformatics/btae090)
    *Development of a high-performance computational toolkit to extract condition-specific or differentially expressed sequences independently of gene annotations.*

* **Integrative Pan-Cancer Genomic and Transcriptomic Analyses of Refractory Metastatic Cancer.** Pradat Y, Viot J, ..., Cournède PH, **Gautheret D**, Nikolaev SI.
  *Cancer Discovery* (2023). [DOI: 10.1158/2159-8290.CD-22-0966](https://doi.org/10.1158/2159-8290.CD-22-0966)
    *A large-scale multi-omic pan-cancer analysis mapping hypothetical and investigational therapy resistance mechanisms.*

* **Cell-cycle-dependent mRNA localization in P-bodies.** Safieddine A, ..., **Gautheret D**, Walter T, Bertrand E, Bénard M, Weil D.
  *Molecular Cell* (2024). [DOI: 10.1016/j.molcel.2024.09.011](https://doi.org/10.1016/j.molcel.2024.09.011)
    *Elucidating the spatial dynamics of mRNA capturing mechanisms within membraneless organelles across the cell cycle.*

---
[daniel.gautheret@universite-paris-saclay.fr]

